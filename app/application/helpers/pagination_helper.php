...
if (!function_exists('create_pagination'))
{
	...
	function create_pagination($uri, $total_rows, $limit = NULL, $uri_segment = 4, $full_tag_wrap = TRUE)
	{
		...
		$ctrl_methods = array('report','users');
	   ...
	   $config['base_url']	= $config['suffix'] !== FALSE ? preg_replace("/{$config['suffix']}/",'',site_url($uri)) : site_url($uri);

		// Count all records
		$config['total_rows'] = $total_rows;
		$config['per_page'] = $limit === NULL ? Settings::get('records_per_page') : $limit;
		$config['uri_segment'] = $uri_segment;
		//$config['page_query_string']	= ( in_array($ci->router->method,$ctrl_methods) )  ? TRUE : FALSE;
